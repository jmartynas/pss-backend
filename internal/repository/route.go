...
func (r *routeRepository) Update(ctx context.Context, id, creatorID uuid.UUID, in domain.UpdateRouteInput) error {
	...
	case ownerIDStr != creatorID.String():
...
func fetchRouteStops(ctx context.Context, db *sql.DB, routeIDs []string) (map[string][]domain.Stop, error) {
	...
	for rows.Next() {
		...
		err = rows.Scan(&routeIDStr, &stopIDStr, &s.Position, &s.Lat, &s.Lng, &s.PlaceID, &s.FormattedAddress, &s.ParticipantID)
		if err != nil {
...
func fetchRouteParticipants(ctx context.Context, db *sql.DB, routeIDs []string) ([]participantWithRoute, error) {
	...
	for rows.Next() {
		...
		err = rows.Scan(&pr.routeID, &userIDStr, &pr.Name, &pr.Status)
		if err != nil {
...
func publishEmailLog(nc *nats.Conn, emailLogID, emailType string) error {
	if nc == nil {
		return fmt.Errorf("nats conn is nil")
	}
	payload := fmt.Sprintf(`{"id":%q,"type":%q}`, emailLogID, emailType)
	_ = nc.Publish("email", []byte(payload))
	return nil
}
