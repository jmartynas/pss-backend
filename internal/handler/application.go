...
func (h *ApplicationHandler) Apply(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		case errors.Is(err, errs.ErrStopPastRouteEnd):
			writeJSON(w, http.StatusBadRequest, map[string]string{"error": "one or more stops is past the driver's destination"})
		case errors.Is(err, errs.ErrStopTooFarFromRoute):
			writeJSON(w, http.StatusBadRequest, map[string]string{"error": "one or more stops is too far from the route"})
		default:
