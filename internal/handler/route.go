...
func (h *RouteHandler) CreateRoute(w http.ResponseWriter, r *http.Request) {
	...
	if in.MaxPassengers < 1 {
		http.Error(w, "max_passengers must be at least 1", http.StatusBadRequest)
		return
	}
	if in.MaxDeviation < 0 {
		http.Error(w, "max_deviation must be 0 or greater", http.StatusBadRequest)
		return
	}
	if in.Price != nil && *in.Price < 0 {
		http.Error(w, "price must be 0 or greater", http.StatusBadRequest)
		return
	}
	...
	if !validCoords(in.StartLat, in.StartLng) || !validCoords(in.EndLat, in.EndLng) {
...
func (h *RouteHandler) UpdateRoute(w http.ResponseWriter, r *http.Request) {
	...
	if in.MaxPassengers != nil && *in.MaxPassengers < 1 {
		http.Error(w, "max_passengers must be at least 1", http.StatusBadRequest)
		return
	}
	if in.MaxDeviation != nil && *in.MaxDeviation < 0 {
		http.Error(w, "max_deviation must be 0 or greater", http.StatusBadRequest)
		return
	}
	if in.Price != nil && *in.Price < 0 {
	...
	if in.StartLat != nil && (!validCoords(*in.StartLat, *in.StartLng) || !validCoords(*in.EndLat, *in.EndLng)) {
		http.Error(w, "invalid coordinates", http.StatusBadRequest)
		return
	}
	if in.Stops != nil {
