...
export default function LoginPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) { setError('Neteisingas el. pašto formatas'); return }
    if (!password) { setError('Įveskite slaptažodį'); return }

    setLoading(true)
    try {
      const res = await login(email, password)
      signIn(res.token, res.admin_id, res.permissions)
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex items-center justify-center">
      <div className="bg-white rounded-xl shadow p-8 w-full max-w-sm">
        <h1 className="text-2xl font-bold text-gray-800 mb-6">PSS Administravimas</h1>
