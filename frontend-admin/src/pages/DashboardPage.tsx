...
export default function DashboardPage() {
  ...
  async function handleChangePw(e: FormEvent) {
    ...
    if (!currentPw) { setPwError('Įveskite dabartinį slaptažodį'); return }
    if (!newPw) { setPwError('Įveskite naują slaptažodį'); return }
    if (newPw.length < 8) { setPwError('Naujas slaptažodis turi būti bent 8 simbolių'); return }
    if (currentPw === newPw) { setPwError('Naujas slaptažodis turi skirtis nuo dabartinio'); return }
    if (newPw !== confirmPw) { setPwError('Slaptažodžiai nesutampa'); return }
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      {showPwForm && (
        <div className="bg-white border-b border-gray-200 px-6 py-4">
          <form onSubmit={handleChangePw} className="flex flex-wrap items-end gap-3">
            <div>
              ...
              <input
                ...
                required
                onInvalid={e => (e.target as HTMLInputElement).setCustomValidity('Šis laukas yra privalomas')}
            ...
            <div>
              <label className="block text-xs text-gray-500 mb-1">Pakartokite naują slaptažodį</label>
              <input
                type="password"
                value={confirmPw}
