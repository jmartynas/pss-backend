...
export default function AdminsTab({ currentAdminId }: { currentAdminId: string }) {
  ...
  async function deleteAdmin(adminId: string) {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Veiksmas nepavyko')
    }
  }
  ...
  async function savePerms(adminId: string) {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Nepavyko išsaugoti teisių')
    } finally {
      setSaving(null)
    }
  }
  ...
  async function handleCreate(e: FormEvent) {
    ...
    if (!newEmail.trim()) { setCreateError('Įveskite el. paštą'); return }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(newEmail)) { setCreateError('Neteisingas el. pašto formatas'); return }
    if (!newPassword) { setCreateError('Įveskite slaptažodį'); return }
    if (newPassword.length < 8) { setCreateError('Slaptažodis turi būti bent 8 simbolių'); return }
