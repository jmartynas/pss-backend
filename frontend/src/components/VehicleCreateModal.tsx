...
export default function VehicleCreateModal({ onCreated, onClose }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.model.trim()) { setError('Įveskite modelį'); return }
    if (!form.plate_number.trim()) { setError('Įveskite valstybinį numerį'); return }
    const seats = parseInt(form.seats, 10)
    if (isNaN(seats) || seats < 1 || seats > 16) { setError('Sėdynių skaičius turi būti nuo 1 iki 16'); return }
