...
export default function CreateRoutePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const maxPassengers = parseInt(effectiveSeats, 10)
    if (isNaN(maxPassengers) || maxPassengers < 1) {
      setError('Keleivių skaičius turi būti bent 1.')
      return
    }
    const maxDeviation = parseFloat(details.max_deviation)
    if (isNaN(maxDeviation) || maxDeviation < 0) {
      setError('Maks. nukrypimas turi būti teigiamas skaičius.')
      return
    }
    if (details.price !== '' && parseFloat(details.price) < 0) {
      setError('Kaina negali būti neigiama.')
      return
    }
    setError(null)
    setLoading(true)
    try {
      ...
      setError(errMsg(err, 'Nepavyko sukurti maršruto'))
    } finally {
      setLoading(false)
    }
  }
