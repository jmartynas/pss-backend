...
export default function ProfilePage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSaveError(errMsg(err, 'Nepavyko išsaugoti'))
    } finally {
      setSaving(false)
    }
  }
  ...
  const handleDisable = async () => {
    ...
    try {
      ...
      setActionError(errMsg(err, 'Nepavyko išjungti paskyros'))
      setDisabling(false)
    }
  }
  ...
  const handleDeleteVehicle = async (id: string) => {
    try {
      ...
      setActionError('Nepavyko ištrinti transporto priemonės')
    }
  }
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8 space-y-6">
      ...
      {actionError && (
        <div className="bg-red-50 text-red-600 rounded-lg p-4 text-sm">{actionError}</div>
      )}
