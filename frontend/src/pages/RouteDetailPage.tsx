...
export default function RouteDetailPage() {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      setActionError(errMsg(e, 'Nepavyko atnaujinti maršruto'))
    } finally {
...
const isCreator = user && route && user.id === route.creator_id
  ...
  useEffect(() => {
    ...
    getRoute(id)
      ...
      .catch(() => setError('Maršrutas nerastas'))
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      setActionError(errMsg(e, 'Nepavyko pateikti prašymo'))
    } finally {
      setActionLoading(false)
    }
  }
...
const handleCancelMyApplication = async () => {
    ...
    try {
      ...
      setActionError(errMsg(e, 'Nepavyko atšaukti'))
    } finally {
      setActionLoading(false)
    }
  }
  ...
  const handleSaveApplication = async () => {
    ...
    try {
      ...
      setActionError(errMsg(e, 'Nepavyko išsaugoti pakeitimų'))
    } finally {
      setActionLoading(false)
    }
  }
  ...
  const handleReview = async (appId: string, status: 'approved' | 'rejected') => {
    ...
    try {
      ...
      setActionError(errMsg(e, 'Veiksmas nepavyko'))
    } finally {
      setActionLoading(false)
    }
  }
  ...
  const handleReviewStopChange = async (appId: string, approve: boolean) => {
    ...
    try {
      ...
      setActionError(errMsg(e, 'Nepavyko'))
    } finally {
      setActionLoading(false)
    }
  }
  ...
  const handleSubmitReview = async (targetUserID: string) => {
    ...
    try {
      ...
      setActionError(errMsg(e, 'Nepavyko išsaugoti įvertinimo'))
    } finally {
      setActionLoading(false)
    }
  }
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-6 mb-6">
        ...
        {isCreator && editingRoute && (
          <div className="mt-4 pt-4 border-t border-gray-100">
            ...
            {actionError && (
              <div className="bg-red-50 text-red-600 rounded-lg p-3 text-sm mt-4">{actionError}</div>
            )}
            <div className="flex gap-2 mt-3">
              ...
              <button
                onClick={() => { setEditingRoute(false); setActionError(null) }}
                disabled={actionLoading}
        ...
        {canApply && (
          <div className="mt-4 pt-4 border-t border-gray-100">
            ...
            />
            {actionError && (
              <div className="bg-red-50 text-red-600 rounded-lg p-3 text-sm mt-3">{actionError}</div>
            )}
            <div className="mt-3">
              <button
                onClick={handleApply}
          ...
          {myApplication.status === 'pending' && editingApplication && (
            <>
              ...
              <div className="flex gap-2 mt-3">
                ...
                <button
                  onClick={() => { setEditingApplication(false); setActionError(null) }}
                  disabled={actionLoading}
                  className="px-5 py-2 rounded-lg text-sm font-medium text-gray-600 hover:text-gray-800 disabled:opacity-50"
                >
                  Atšaukti
          ...
          {changingStops && (
            <>
              ...
              <div className="flex gap-2 mt-3">
                ...
                <button
                  onClick={() => { setChangingStops(false); setChangeAllStops([]); setChangeComment(''); setActionError(null) }}
                  disabled={actionLoading}
                  className="px-5 py-2 rounded-lg text-sm font-medium text-gray-600 hover:text-gray-800 disabled:opacity-50"
                >
                  Atšaukti
